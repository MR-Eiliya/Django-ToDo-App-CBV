...
class TestTaskAPIView:
    ...
    def test_create_task_with_invalid_data(self, api_client, common_user):
        url = reverse("todo:api-v1:task-list")
        data = {"titllee": "test task"}
        user = common_user
        api_client.force_authenticate(user=user)
        response = api_client.post(url, data)
        assert response.status_code == 400

    def test_task_list_returns_all_tasks_for_authenticated_user(
    ...
    def test_task_detail_returns_task_for_authenticated_user(
        self, api_client, common_user
    ):
    ...
    def test_retrieve_task_detail_for_owner(self, api_client, common_user):
        task = Task.objects.create(title="task title", user=common_user)
        url = reverse("todo:api-v1:task-detail", kwargs={"pk": task.id})
        api_client.force_authenticate(user=common_user)
        response = api_client.get(url)
        assert response.status_code == 200

    def test_retrieve_task_detail_for_none_owner(
        self, api_client, common_user, sub_user
    ):
        task = Task.objects.create(title="private task", user=common_user)
        url = reverse("todo:api-v1:task-detail", kwargs={"pk": task.id})
    ...
    def test_update_task_with_invalid_data(self, api_client, common_user):
        task = Task.objects.create(title="task title", user=common_user)
        url = reverse("todo:api-v1:task-detail", kwargs={"pk": task.id})
        data = {"tittlee": "task title updated"}
        api_client.force_authenticate(user=common_user)
        response = api_client.put(url, data)
        assert response.status_code == 400
        task.refresh_from_db()
    ...
    def test_delete_task_for_owner(self, api_client, common_user):
        task = Task.objects.create(title="for delete", user=common_user)
        api_client.force_authenticate(user=common_user)
        url = reverse("todo:api-v1:task-detail", kwargs={"pk": task.id})
    ...
    def test_retrieve_task_without_authentication(self, api_client, common_user):
        task = Task.objects.create(title="test", user=common_user)
        url = reverse("todo:api-v1:task-detail", kwargs={"pk": task.id})
        response = api_client.get(url)
        assert response.status_code == 401
